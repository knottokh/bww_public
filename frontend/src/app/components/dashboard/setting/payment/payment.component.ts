...
import { AuthenticationService, PaymentService } from "src/app/_services";

@Component({
  selector: "app-payment",
  templateUrl: "./payment.component.html",
  styleUrls: ["../setting.component.scss", "./payment.component.scss"],
})
export class PaymentComponent implements OnInit {
  ...
  fetchData(search) {
    ...
    this.paymentService.findBy(search).subscribe((d: any) => {
  ...
  onSaveData() {
    ...
    let service = this.paymentService.add(
    ...
    if (this.f._id.value && this.f._id.value !== "") {
      isnew = false;
      service = this.paymentService.update(
        this.f._id.value,
        Object.assign(shareData, {
  ...
  swapIndex(a, m, n){
    ...
    [a[m], a[n]] = [a[n], a[m]];
    //this.changeDetectorRef.detectChanges();
    this.paymentService.update_sequence(a).subscribe(
      (data: any) => {
        this.onSearchClick();
      },
      (error: any) => {
  ...
  onDelete() {
    ...
    this.dlgerror = "";
    this.paymentService.delete(this.deleteElement._id).subscribe(
      (data: any) => {
        this.onSearchClick();
        this.decline();
      },
