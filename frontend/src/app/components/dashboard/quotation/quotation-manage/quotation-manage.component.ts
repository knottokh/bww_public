...
import { RouterPath, Unit, UnitEng, User } from "src/app/_models";
import {
...
import { QStockType } from "src/app/_models/qstatus";
...
@Component({
  ...
  styleUrls: ["./quotation-manage.component.scss"],
})
export class QuotationManageComponent implements OnInit {
  ...
  onBackto() {
    if (this.paramId === "new") {
      ...
      this.router.navigate([
        `/${this.routepath.quotation}/${this.dataSourceQuotation._id}/view`,
      ]);
    }
